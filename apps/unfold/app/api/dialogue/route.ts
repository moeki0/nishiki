...
const mainFlow = g.flow([
  ...
  g.loop([
    ...
    g.pick(timelineSchema, compareSchema, sourceSchema, peopleSchema),
  ]),
  g.prose(
    "Wrap up with background context, underlying causes, or lasting consequences",
  ),
  deepdiveInline,
  boldInline,
  footnoteInline,
...
const visualFlow = g.flow([g.pick(agelineSchema, animapSchema)]);
...
export async function POST(request: Request) {
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      const [emphasisText, interactiveText, visualText, wrapupText] =
        await Promise.all([
